...
class MediaLibraryQueries {
  ...
  List<MediaFile> get uncategorized => _catalog.mediaFiles
      .where(
        (file) =>
            (file.mediaType == entity.StoredMediaType.movie && file.movieTmdbId == null) ||
            (file.mediaType == entity.StoredMediaType.episode && file.episodeTmdbId == null),
      )
  ...
  List<LibraryItem> get recentlyAddedContent {
    ...
    for (final movie in _catalog.movies) {
      ...
      final relatedFiles = _catalog.mediaFiles.where((file) => file.movieTmdbId == movie.tmdbId);
    ...
    for (final show in _catalog.tvShows) {
      ...
      final relatedFiles = _catalog.mediaFiles.where((file) => file.tvShowTmdbId == show.tmdbId);
      if (relatedFiles.isEmpty) continue;
      final earliestDate = relatedFiles.map((file) => file.addedAt).reduce((a, b) => a.isBefore(b) ? a : b);
  ...
  Set<String> get _availableMovieTmdbIds => _catalog.mediaFiles
      .where(
        (file) =>
            file.mediaType == entity.StoredMediaType.movie && file.movieTmdbId != null && file.movieTmdbId!.isNotEmpty,
      )
  ...
  MediaCardViewData _buildCard(MediaFile file, {bool useBackdrop = false}) {
    ...
    if (file.mediaType == MediaType.movie && file.movieTmdbId != null) {
      final index = _catalog.movies.indexWhere((movie) => movie.tmdbId == file.movieTmdbId);
      if (index >= 0) {
        libraryItem = MediaEntityMapper.toMovie(_catalog.movies[index]);
      }
    } else if (file.mediaType == MediaType.episode && file.tvShowTmdbId != null) {
      final index = _catalog.tvShows.indexWhere((show) => show.tmdbId == file.tvShowTmdbId);
      if (index >= 0) {
